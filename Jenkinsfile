    ...
    stages {
        ...
        stage('Checkout on Jenkins Controller') {
            ...
            steps {
                ...
                stash name: 'argocd-repo', includes: '**/*'
            }
        }

        stage('Install Argo CD from Kubernetes Agent') {
            agent {
                kubernetes {
                    ...
                    steps {
                        container('kubectl') {
                            script {
                                ...
                                env.ARGOCD_MANIFEST_PATH = sh(
                                    script: "grep -A10 '^argocd:' ${CLUSTER_FILE} | awk '/install_manifest_path:/ {print \$2; exit}'",
                                    returnStdout: true
                                ).trim()
                            ...
                            echo "Argo CD manifest path: ${ARGOCD_MANIFEST_PATH}"
                    ...
                    steps {
                        container('kubectl') {
                            sh '''
                                ...
                                if [ -z "${ARGOCD_MANIFEST_PATH}" ]; then
                                  ARGOCD_MANIFEST_PATH="install.yaml"
                                fi
                        ...
                        container('kubectl') {
                            withCredentials([
                                ...
                                sh '''
                                    ...
                                    echo "Manifest file: ${ARGOCD_MANIFEST_PATH}"
